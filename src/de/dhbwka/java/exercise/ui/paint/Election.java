...
public class Election extends JComponent {
    ...
    protected void paintComponent(Graphics g) {
        ...
        g.drawString(INFO_TEXT, width - 40, 23);
        ...
        for (int i = 1; i <= (height - (height - belowBars) - 42); i++) {
            int y = (int) (height - (height - belowBars) - i * 10 * heightPerPercent);
            g.setColor(Color.white);
            g.fillRect(0, y, width, 1);
            g.setColor(Color.black);
            g.drawString(Integer.toString(i*10), 0, y-1);
        }
    ...
    private void drawParties(Graphics g, int width, int height, double heightPerPercent, int belowBars) {
        ...
        for (int i = 0; i < parties.length; i++) {
            ...
            //horizontal
            g.fillRect((MARGIN_LEFT + 10 + maxBarWidth * i + 1), (belowBars - (int) (heightPerPercent * parties[i].getPercent()) - 1), maxBarWidth - 20, 1);
            //vertical
            g.fillRect((MARGIN_LEFT + maxBarWidth - 10 + maxBarWidth * i), (belowBars - (int) (heightPerPercent * parties[i].getPercent())), 1, (int) (heightPerPercent * parties[i].getPercent()) - 1);
            ...
            //percentage
            g.drawString(Double.toString(parties[i].getPercent()).replaceAll("\\.", ","), MARGIN_LEFT + 10 + i * maxBarWidth, belowBars + 62); //40-seeAbove 2-darkblue small horizontal bar below white bar 20-small space between bars(4) and number(16)
        }
    }
